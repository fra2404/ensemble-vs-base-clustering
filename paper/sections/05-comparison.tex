...
Figure \ref{fig:metrics_mall} shows the validation metrics comparison for the Mall Customers dataset, where K-Means achieves the highest overall performance. Similar comparative plots for Customer Personality and Wholesale Customers datasets are available among the generated figures in the code repository, demonstrating how algorithm rankings change across different data characteristics.
...
The PCA projections for Mall Customers reveal distinct clustering patterns for different algorithms:
\begin{itemize}
    \item \textbf{K-Means (Figure~\ref{fig:kmeans_pca})}: Five clusters display clear separation with some overlap between adjacent groups. Colors represent different customer segments based on age, income, and spending patterns.
    \item \textbf{CSPA ensemble (Figure~\ref{fig:cspa_pca})}: The consensus-based approach produces more compact and well-separated clusters compared to individual algorithms, effectively reducing noise in cluster boundaries.
    \item \textbf{HGPA ensemble (Figure~\ref{fig:hgpa_pca})}: The hypergraph-based method shows the most distinct cluster separation with minimal overlap between groups, indicating high clustering confidence.
    \item \textbf{MCLA ensemble (Figure~\ref{fig:mcla_pca})}: The meta-clustering approach produces balanced cluster sizes with clear boundaries, offering a compromise between individual algorithm performance and ensemble stability.
\end{itemize}

All figures in this section show results for the Mall Customers dataset as a representative example.\footnote{Similar visualizations are available for Customer Personality and Wholesale Customers datasets, showing how clustering patterns vary across different data domains and algorithm performance depends on dataset characteristics.}
